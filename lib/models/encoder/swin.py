...
from .utils import (find_pruneable_heads_and_indices, meshgrid,
                    prune_linear_layer)
...
def window_partition(input_feature, window_size):
    ...
    windows = input_feature.permute(0, 1, 3, 2, 4, 5).contiguous(
...
def window_reverse(windows, window_size, height, width):
    ...
    windows = windows.view(-1, height // window_size, width //
...
class SwinTransformer(nn.Module):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False, **kwargs):
        ...
        self.layernorm = nn.LayerNorm(
            self.num_features, eps=config.layer_norm_eps)
